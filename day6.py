...
def execute_instructions(state, turn_on, turn_off, toggle):
    ...
    for line in instructions:
        cmd, start_coords, end_coords = parse_line(line)
        op = ops[cmd]
...
def part1():
    def turn_on(state, idx):
        state[idx] = True

    def turn_off(state, idx):
        state[idx] = False
    ...
    print("Number of lights lit: {}".format(state.count(True)))
...
def part2():
    ...
    print("Total brightness: {}".format(sum(state)))
