...
def execute_instructions(state, turn_on, turn_off, toggle):
    ops = [turn_on, turn_off, toggle]
...
def part1():
    def turn_on(state, x_range, y_range):
        state[x_range, y_range] = True

    def turn_off(state, x_range, y_range):
        state[x_range, y_range] = False

    def toggle(state, x_range, y_range):
        numpy.logical_not(state[x_range, y_range], out=state[x_range, y_range])

    state = numpy.zeros((1000, 1000), dtype=numpy.bool)
    execute_instructions(state, turn_on, turn_off, toggle)

    print("Number of lights lit: {}".format(numpy.count_nonzero(state)))
...
def part2():
    ...
    def turn_off(state, x_range, y_range):
        state[x_range, y_range] -= 1
        numpy.maximum(state[x_range, y_range], 0, out=state[x_range, y_range])

    def toggle(state, x_range, y_range):
        state[x_range, y_range] += 2

    state = numpy.zeros((1000, 1000), dtype=numpy.int)
    execute_instructions(state, turn_on, turn_off, toggle)

    print("Total brightness: {}".format(numpy.sum(state)))
